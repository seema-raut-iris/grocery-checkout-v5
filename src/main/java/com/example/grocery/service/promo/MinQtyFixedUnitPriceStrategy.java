 ...
 * Example: (APPLES, minQty=3, discountedUnitPrice=£0.55)
 * Discount = (normalUnitPrice - discountedUnitPrice) * quantity
 */
public class MinQtyFixedUnitPriceStrategy implements DiscountStrategy {
    ...
    public MinQtyFixedUnitPriceStrategy(ItemType targetItem, int minQty, BigDecimal discountedUnitPrice) {
        if (minQty <= 0) throw new IllegalArgumentException("minQty must be > 0");
        if (discountedUnitPrice == null || discountedUnitPrice.compareTo(BigDecimal.ZERO) <= 0)
            throw new IllegalArgumentException("discountedUnitPrice must be positive");

        this.targetItem = Objects.requireNonNull(targetItem);
        this.minQty = minQty;
        this.discountedUnitPrice = scale(discountedUnitPrice);
    ...
    public DiscountResult apply(ItemType type, int quantity, BigDecimal unitPrice) {
        if (type != targetItem || quantity < minQty) {
    ...
    @Override public String name() { return targetItem.name().toLowerCase() + "-min-" + minQty + "-unit-" + discountedUnitPrice; }
}
